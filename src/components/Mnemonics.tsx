...
export default function Mnemonics( { mnemonic }: MnemonicsProps ) {
    ...
    return(
        <div className="w-[360px] md:w-[480px] p-8 bg-slate-800 rounded-xl">
            ...
            <div className="flex flex-wrap gap-2 pt-6 pb-4 items-center justify-center">
                {wordsArray.map((word, index) => (
                    <div 
                        key={index} 
                        className="bg-gray-700 text-white w-20 md:w-24 py-2 px-4 rounded-lg shadow-sm text-center"
                    >
                        {word}
                    </div>
                ))}
